  ...
  render_weekly_table(data) {
    ...
    for (let item of group_items) {
      ...
      for (let _item of item.a_id) {
        ...
        tag += `<tr><td style="text-align: right;">${this.get_article_name(_item)}</td>${sub_tag}<td class="bg-soft-primary">$${sub_total.toFixed(2)}</td><td class="bg-soft-primary">${group_total != 0 ? (sub_total / group_total * 100).toFixed(2) + '%' : '0%'}</td></tr>`
      }
      tag += `
