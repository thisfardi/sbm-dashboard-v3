...
export class DashboardComponent implements OnInit {
    ...
    */
    ...
    private _fetchSumData(){
        ...
        this.apiService.sum_data(this.parseService.encode({
            from: moment(this.dash_date_range['to']).subtract(9, 'days').format('YYYY-MM-DD'),
            to: this.dash_date_range['to'],
                    ...
                    if(data['status'] == 'success'){
                        ...
                        for(let item of sum_data.transaction){
                            ...
                            if(this._10_days.indexOf(item.d) > -1){
                                ...
                                this.sum_data['average_bill'][this._10_days.indexOf(item.d)] = parseFloat(item.average_bill ? item.average_bill : 0);
                            }
                        }
                        // Get tip
    ...
    */
    refresh_values(){
        ...
        let empty_values = [];
        for(let i = 10; i > 0; i--){
            this._10_days.push(moment(this.dash_date_range['to']).subtract(i - 1, 'days').format('YYYY-MM-DD'));
            empty_values.push(0);
        }
        this.sum_data = {
    ...
    apply_filter(){
        ...
        this.dash_date_range['from'] = moment(this.dash_date_range['from']).format('YYYY-MM-DD');
        if(!this.dash_date_range['to']){
            this.dash_date_range['to'] = moment(this.dash_date_range['from']).format('YYYY-MM-DD');
        }else{
            this.dash_date_range['to'] = moment(this.dash_date_range['to']).format('YYYY-MM-DD');
        }
        this.refresh_values();
        this._fetchSumData();
    }
